...
class DataBaseHandle(object):
    ...
    def insertDB(self,sql):
        ...
        try:
            ...
            return self.cursor.execute(sql)  # 返回 插入数据 条数 可以根据 返回值 判定处理结果
            # print(tt)
            self.db.commit()
        except:
            # 发生错误时回滚
    ...
    def deleteDB(self,sql):
        ...
        try:
            # 执行sql
            self.cursor.execute(sql)
            return self.cursor.execute(sql) # 返回 删除数据 条数 可以根据 返回值 判定处理结果
            # print(tt)
            self.db.commit()
        except:
            # 发生错误时回滚
    ...
    def updateDb(self,sql):
        ...
        try:
            # 执行sql
            self.cursor.execute(sql)
            return self.cursor.execute(sql) # 返回 更新数据 条数 可以根据 返回值 判定处理结果
            # print(tt)
            self.db.commit()
        except:
            # 发生错误时回滚
